...
import { FaVideo, FaPlay, FaExclamationTriangle, FaCheckCircle, FaSpinner } from 'react-icons/fa';
...
const HlsBatchConverter = () => {
    ...
    const fetchStatus = async () => {
        try {
            const res = await LMSService.getHlsBatchStatus();
            if (res && res.success) {
                setStatus(res.data);
            }
        } catch (e) {
    ...
    const handleStartBatch = async () => {
        const msg = force 
            ? 'Deseja forçar a reconversão de TODOS os vídeos do LMS para HLS? Esse processo pode demorar dependendo da quantidade de vídeos.'
            : `Deseja iniciar a conversão em lote de ${status.pending} vídeo(s) pendente(s) no LMS?`;
        ...
        setSubmitting(true);
        try {
            const res = await LMSService.convertHlsBatch(force);
            if (res && res.success) {
                setStatus(prev => ({
                    ...prev,
                    is_running: true,
            ...
            alert('Falha ao iniciar conversão: ' + e.message);
        } finally {
            setSubmitting(false);
        }
    };
    ...
    return (
        <Card>
            <Title>
                <FaVideo /> Conversão HLS em Lote
            </Title>
            <Grid>
                <StatsGroup>
                    <StatItem>
                        <span className="value">{status.total_videos}</span>
                        <span className="label">Total de Vídeos</span>
                    </StatItem>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.25rem' }}>
                    ...
                    >
                        {status.is_running ? (
                            <>
                                <FaSpinner className="spin-icon" /> Convertendo...
                            </>
                        ) : (
                            <>
                                <FaPlay /> Iniciar Conversão
                            </>
                        )}
                    </Button>
            ...
            {!status.is_running && status.pending === 0 && (
                <div style={{ marginTop: '1.2rem', display: 'flex', alignItems: 'center', gap: '0.5rem', color: '#10B981', fontSize: '0.875rem', fontWeight: '700' }}>
                    <FaCheckCircle fontSize="1.1rem" /> Todos os vídeos estão convertidos e otimizados para HLS.
                </div>
            )}
...
export default HlsBatchConverter;
